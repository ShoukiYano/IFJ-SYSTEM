...
export const InvoiceDocument = ({ invoice, company }: any) => {
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        <View style={styles.invoiceInfo}>
          ...
          <Text>登録番号: {invoice?.registrationNumber || company?.registrationNumber || ""}</Text>
        </View>

        <View style={styles.dateInfo}>
          <Text>発行日: {safeFormatDate(invoice.issueDate)}</Text>
          <Text>お支払期限: {safeFormatDate(invoice.dueDate)}</Text>
        </View>

        <Text style={styles.title}>御 請 求 書</Text>

        <View style={styles.clientSection}>
          <Text style={{ fontSize: 14 }}>{invoice.client.name} {invoice.client.honorific}</Text>
          {invoice.client.department && <Text>{invoice.client.department}</Text>}
          {invoice.client.manager && <Text>{invoice.client.manager} 様</Text>}
        </View>

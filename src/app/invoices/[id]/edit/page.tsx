  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // Fetch initial assignees and staffs
        if (invoiceData.clientId) {
          const [assigneesRes, staffsRes] = await Promise.all([
            fetch(`/api/assignees?clientId=${invoiceData.clientId}`),
            fetch(`/api/staff?clientId=${invoiceData.clientId}`)
          ]);
          if (assigneesRes.ok) setAssignees(await assigneesRes.json());
          if (staffsRes.ok) setStaffs(await staffsRes.json());
        }
      } catch (error) {

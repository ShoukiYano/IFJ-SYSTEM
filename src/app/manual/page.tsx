...
const SECTIONS: Section[] = [
  ...
  // ─── 3. 取引先管理 ───
  {
    ...
    content: (
      <div className="space-y-6">
        <p className="text-slate-600">請求先企業と、そこに派遣しているエンジニアの情報を管理します。</p>

        <div className="space-y-3">
          <h3 className="font-black text-slate-700">➕ 取引先（企業）を登録する</h3>
          <div className="space-y-3">
        ...
        <div className="space-y-3">
          <h3 className="font-black text-slate-700">✏️ 情報を編集・削除する</h3>
          <div className="space-y-3">
            <Step num={1}>取引先カードの <Badge color="slate">編集</Badge> ボタンをクリックして編集モーダルを開きます。</Step>
            <Step num={2}>内容を変更して「保存」します。</Step>
            <Step num={3}>削除する場合はモーダル内の「削除」ボタンをクリックし、確認後に実行します。</Step>
          </div>
          <Warn>取引先を削除すると、紐づくエンジニア情報も削除されます。請求書データは残ります。</Warn>
        </div>
      </div>
    ),
  },
